...
func main() {
	...
	// Find the keyid to objectid mapping
	keyid := "19"
	oid, err := ctx.FindObjectByID(keyid)
	check(err)

	log.Println("oid:", oid)

	block, err := aes.NewCipher(ctx, 2)
	check(err)

	in := encryptTests[0].in

	expected := encryptTests[0].out

	ciphertext := make([]byte, len(in))

	block.Encrypt(ciphertext, in)

	log.Printf("%x\n", ciphertext)
	log.Printf("%x\n", expected)

	log.Println(bytes.Equal(expected, ciphertext))

	plaintext := make([]byte, len(in))
	block.Decrypt(plaintext, expected)

	log.Printf("%x\n", plaintext)
	log.Printf("%x\n", in)

	log.Println(bytes.Equal(in, plaintext))
